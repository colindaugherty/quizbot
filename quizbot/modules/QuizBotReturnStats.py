...
class QuizBotReturnStats:
    def __init__(self, botname, groupid):
        ...
        c.executemany("SELECT requests FROM stats WHERE (name=? AND groupid=?)", t)
        requests = c.fetchone()
        message += "Total requests to {}: {}\n".format(botname, requests)
        c.executemany("SELECT responses FROM stats WHERE (name=? AND groupid=?)", t)
        responses = c.fetchone()
        message += "Total responses to {}: {}\n".format(botname, responses)
        c.executemany("SELECT FredResponses FROM stats WHERE (name=? AND groupid=?)", t)
        FredResponses = c.fetchone()
        message += "Total FredResponses to {}: {}\n".format(botname, FredResponses)
        c.executemany("SELECT TotalMessages FROM stats WHERE (name=? AND groupid=?)", t)
